namespace Modding {
  ...
  interface PatchMap<C extends ClassHandle>
    extends Map<
      C,
      Map<ClassMethod<C>, { mods: Set<Mod>; patcher: Patcher<C> }>
    > {}
  ...
  type BeforePatch<M extends Function> = (
    ...args: Parameters<M>
  ) => Parameters<M> | undefined;
  type AfterPatch<M extends Function> = (
    returnValue: ReturnType<M>,
    ...args: Parameters<M>
  ) => ReturnType<M> | undefined;
  type PatchedMethod<M extends Function> = (
    ...args: Parameters<M>
  ) => ReturnType<M>;
  type ReplacePatch<M extends Function> = (
  ...
  interface ModContext {
    gameData: {
      ...
      buildPackage: (
        builder: (packageBuilder: Modding.GameDataPackageBuilder) => void
      ) => { package: GameDataPackage; add: () => Promise<void> };
    };
    characterStorage: {
    ...
    settings: {
      section: (name: string) => {
        ...
        add: (
          config:
            | Modding.Settings.SettingConfig
            | Modding.Settings.SettingConfig[]
        ) => void;
      };
    ...
    patch: <C extends ClassHandle>(
      _class: C,
      methodName: ClassMethod<C>
    ) => Modding.Patch<C, ClassMethod<C>>;
  ...
  type GameDataDataObject<K extends keyof ConcreteGameData> =
    ConcreteGameData[K] extends Array<infer D> ? D : ConcreteGameData[K];
  ...
  type GameDataModificationsDataObject<
    K extends keyof ConcreteGameDataModifications
  > = ConcreteGameDataModifications[K] extends Array<infer D>
    ? D
    : ConcreteGameDataModifications[K];
  ...
  type GameDataProperty<T> = keyof T extends infer K
    ? K extends unknown
      ? { [I in keyof T as unknown extends T[I] ? I : never]?: never } & {
          [I in keyof T as unknown extends T[I] ? never : I]-?: (
            data: T[I]
          ) => void;
        }
      : never
  ...
  interface ConcreteGameDataAndModifications
    extends ConcreteGameData,
      ConcreteGameDataModifications {}

  interface GameDataPackageBuilderProperty<
    K extends keyof ConcreteGameDataAndModifications
  > extends GameDataProperty<GameDataAdd<K>>,
      GameDataProperty<GameDataModify<K>> {}
